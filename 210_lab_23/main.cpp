...
int select_goat(list<Goat>);     // Function prototypes for selction, deletion, adding, displaying & main menu
...
int main() {
    ...
    while (again) {
        ...
        if (again) {
            if (choice == 1) {    // if user selected 1, a gaot is added to the list
            ...
            if (choice == 2) {    // if user selected 2, a goat will be removed
                if (trip.empty())
                    cout << "List is empty" << endl;    // message if list is empty
                else
                    delete_goat(trip);    // delete_goat function call
            }
            if (choice== 3) {    // if user selected 2, list is printed
                if (trip.empty())
                    cout << "List is empty" << endl;   // message if list is empty
                else {
    ...
    trip.clear();    // list is cleared at end
    if (trip.empty())
...
void add_goat(list<Goat> &trip, string c[], string n[]) {
    ...
    Goat goat(name, age, color);    //Goat is constructed and added to the list
    trip.push_back(goat);
}
...
void delete_goat(list<Goat> &trip) {
    ...
    cout << "Which Goat do you wish to delete?" << endl;    //user asked the user which goat to delete from the list
    
    index = select_goat(trip);
    
    while (index > trip.size() || index < 0) {    // validation loop to ensure user index entry is in range
        cout << "----------------------------" << endl;
    ...
    list<Goat>::iterator it;    // iterator to navigate the list, initlized to the start of the list
    it = trip.begin();
    ...
    cout << "Updated list" << endl;    // Updatd list printed
    if (trip.empty())
        cout << "[Empty]" << endl;
    else
        display_trip(trip);
...
void display_trip(list<Goat> trip) {
    int count = 1;    // count to number each element in the list when printed
